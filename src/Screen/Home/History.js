    ...
    const fetchHistory = async () => {
      try {
        const response = await fetch(
          `https://localhost:7097/api/BlogPostHistory/${user ? userName : ""}`,
          {
            headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-gray-800 mt-40 mb-40">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-semibold text-white">Blog Post History</h1>
        <Link
          to={`/comments-history`}
        ...
        <table className="w-full table-auto border-collapse">
          <thead>
            <tr className="bg-gray-200">
              <th className="px-4 py-2 p-3">Image </th>
            ...
            {history.map((entry) => (
              <tr key={entry.id} className="border-b">
                <td className="px-4 py-2 text-white">
                  <img
                    src={`https://localhost:7097/Resources/${entry.imageUrl}`}
                    alt={entry.imageUrl}
                ...
                <td className="px-4 py-2 text-white">{entry.body}</td>
                <td className="px-4 py-2 text-white">
